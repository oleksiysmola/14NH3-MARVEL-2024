...
df = pd.read_csv("20CaCeBeCaWithoutHeader.txt", delim_whitespace=True)

df["Assigned"] = df.notna().all(axis=1)
df["Uncertainty"] = 0.01
df["Uncertainty2"] = 0.01

unassigned = df[df["Assigned"] == False] 
unassigned = unassigned.to_string(index=False)
unassignedFile = "Unassigned20CaCeBeCa.txt"
...
marvelFile = "Assigned20CaCeBeCaMarvel.txt"
