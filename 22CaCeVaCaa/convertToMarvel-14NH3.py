...
df = pd.read_csv("Recommended22CaCeVaCaaWithoutHeader.txt", delim_whitespace=True)
...
df = df.reset_index()
df["Source"] = pd.DataFrame({"Source": [f"22CaCeVaCaa.{i + 1}" for i in range(len(df))]})
df = df.drop("index", axis=1)
print(df.head(15).to_string(index=False))
df = df.to_string(index=False)
marvelFile = "AssignedRecommended22CaCeVaCaaMarvel.txt"
